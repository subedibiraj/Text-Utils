...
export default function Textform(props) {
  ...
  return (
    ...
    <h1>{props.heading}</h1>
  <div className="mb-3">
    <textarea className={`form-control text-${props.mode === 'dark' ? 'light' : 'dark'}`} id="myBox" onChange={handleOnChange} rows="8" value = {text} style={{backgroundColor : props.mode === 'dark' ? 'grey' : 'white'}}></textarea>
  </div>
  ...
  <button className="btn btn-primary mx-2 my-1" onClick = {handleCoClick}>Copy Text</button>
  <button className="btn btn-danger mx-2 my-1" onClick = {handleClClick}>Clear Text</button>
